...
// Wake word detection using MicroWakeWord.
//
// Placeholder: uses VAD speech onset as the trigger until model files are
// placed in components/micro_wake_word/.
class MwwWakeWord : public WakeWord {
public:
    MwwWakeWord();
    ~MwwWakeWord();
...
private:
    ...
    std::atomic<int>                         warmup_samples_left_{0};
